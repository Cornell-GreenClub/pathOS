...
- **CORS Policy:** Explicitly restrict your backend API to allow requests only from your specific frontend domain.
- **"Keep-Awake" Risk:** If an external actor pings the IP constantly, the CloudWatch "Sleep" trigger will not fire. 
- **Hardening:** For production environments, place OSRM behind an Nginx proxy on the instance to validate API keys locally before they hit the OSRM engine.
