...
df_static = df_static[df_static['Generalized_Weight'] != 'NO DATA']
df_static['Generalized_Weight'] = pd.to_numeric(df_static['Generalized_Weight'])
df_static['Weight_kg'] = df_static['Generalized_Weight'] * 0.453592
df_weights = df_static[['VehId', 'Weight_kg']]
...
df = pd.read_csv('final_ice_trips_master.csv', usecols=cols_to_use, low_memory=False)
print(f"Loaded {len(df):,} rows")

# Merge weights
df = pd.merge(df, df_weights, on='VehId', how='inner')
...
def aggregate_trip(trip_df):
    ...
    if len(elev_diffs) > 0:
        ...
        elevation_loss_m = np.sum(elev_diffs[elev_diffs < 0])  # will be negative
    else:
        elevation_gain_m = 0
        elevation_loss_m = 0
...
# Save both versions
df_trips.to_csv('/Users/fli6/Desktop/Projects/pathos_model_updated/Data/trip_summaries_all.csv', index=False)
df_clean.to_csv('/Users/fli6/Desktop/Projects/pathos_model_updated/Data/trip_summaries_clean.csv', index=False)

print(f"\n✓ Saved 'trip_summaries_all.csv' ({len(df_trips):,} trips)")
print(f"✓ Saved 'trip_summaries_clean.csv' ({len(df_clean):,} trips)")

# ============================================================
# 7. PREVIEW FOR MODELING
# ============================================================

print(f"\n{'='*50}")
print("PREVIEW: READY FOR MODELING")
print(f"{'='*50}")

print("\nFeatures available for modeling:")
print("  - Total_Distance_km")
print("  - Avg_Speed_kmh")
print("  - Elevation_Gain_m")
print("  - Elevation_Gain_per_km (hilliness)")
print("  - Weight_kg")

print("\nTarget variable:")
print("  - Total_Fuel_Liters (or Fuel_L_per_100km)")

print("\nSample of clean data:")
print(df_clean[['VehId', 'Trip', 'Total_Distance_km', 'Avg_Speed_kmh', 
                'Elevation_Gain_m', 'Weight_kg', 'Total_Fuel_Liters', 
                'Fuel_L_per_100km']].head(10).to_string())